function CreatePreviewHead(){
	...
	*/
	...
	function publicSinglePageHeadInfo(kid,title,target){
		var config={
			...
			$.ajax({
				...
				data:{method:'singlePageTableInfo', kid:kid},
				async:false,
				dataType:'json',
	...
	function registrationPageHead(kid,title,target){
		...
		if(config.title=='介入诊断治疗申请单'||config.title=='出院记录'){
			head.ctxStyle.titleStyle = 'left:0;top:67px;width:100%;font-size:22px;font-weight:bold;text-align:center;position:absolute;';
			head.ctxStyle.pageNoStyle = 'left:0;top:92px;width:100%;height:12px;font-size:12px;text-align:center;position:absolute;';
			head.ctxStyle.pIntimesStyle= 'left:50px;top:92px;width:120px;height:12px;font-size:12px;position:absolute;';
		}*/
		head.renderTo=config.renderTo;
		head.title=config.title;
		head.pName=config.patientName;
