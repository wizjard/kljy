function RepoertHeader(){
	...
	this.create=function(){
		...
		var html=	
					...
					'		<tr>'+
					'			<td style="width:150px; text-align:left; text-indent:60px; vertical-align:text-bottom; font-size:12px;">第&nbsp;<span id="outCount">{outCount}</span>&nbsp;次门诊</td>'+
	      ...
	      if(xmlHttp.readyState==4)
	        {
			 ...
			 var obj=eval('('+res+')');
			 html=html.replace('{outCount}',obj.outCount).replace('{patientName}',obj.patientName).replace('{patientNo}',obj.patientNo);
	         el.innerHTML=html;
			}
	      }
	    xmlHttp.open("GET",this.basePath+"/common/CommonAction.do?method=GetHeaderInfomenzhen&pid="+this.pid+"&cid="+this.cid+"&timestamp="+new Date().getTime(),true);
	    xmlHttp.send(null);
