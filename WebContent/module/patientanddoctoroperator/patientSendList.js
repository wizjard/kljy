...
Ext.onReady(function(){
	new Ext.Viewport({
	   ...
	   // if(top[i].length>0) {
	    //    alert($(top.dom).attr("id"));
	    //}
	
});

var checkCurrentDeptOrApplication= 1;//申请科室
function createGrid(){
	...
	var _cm_normal=[
		...
		//{header:'申请人', dataIndex:'optrole'},
		{header:'当前申请人', renderer: function(value, meta, record, rowIndex, colIndex, store){
		    if(record.data.optrole=='doctor') {
		        return "责任医生";
		    } else if(record.data.optrole=='member') {
		        return "会员";
		    } else {
		        return "会员";
		    }
		...
		{header:'操作',renderer:function(value, meta, record, rowIndex, colIndex, store){
			if(checkCurrentDeptOrApplication == 0){
				...
				if(record.data.autoFlag == 0){
				    if(record.data.flag ==0 && record.data.optrole != 'member') {
						return "<a href='###' onclick='memberHandler("+record.data.id+",1)'>同意</a>&nbsp;&nbsp;&nbsp;&nbsp;" + 
						"<a href='###' onclick='memberHandler("+record.data.id+",2)'>不同意</a>";
	...
	var _baseParams={
		method:'findUserHistoryApplicationRecord',
		//deptCode:'GB4',
		flag:0,//申请科室
              ...
              ,
               '-',
               {
            	  text:'转入申请',handler:function(){
      				...
      				_store.baseParams={method:'findUserHistoryApplicationRecord',
      					//deptCode:'GB4',
						flag:0,//申请科室
						patientId:top.Member.patientId
					};
...
//会员转科
function memberHandler(_this, state) {
        ...
        buttons:[
            {    
                text:'保存',handler:function(){
                    ...
                    _values.id = _this;
                    _values.flag = state;
                    //alert(_this + "  " + state + ":==  " + Ext.util.JSON.encode(_values));
                    Ext.Ajax.request({
