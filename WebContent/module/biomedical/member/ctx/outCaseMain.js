...
function currentTime()
{
	...
	var currentTime = year+"-"+month+"-"+day+" "+hours+":"+minute;
	return currentTime;
}
...
function linkPlan(omcid) {
	var planURL = App.App_Info.BasePath+'/module/plan/selectPlan.html?paId='+PID + "&omcId=" + omcid + "&type=omc&KID=" + KID + "&outRegno=" + outRegno;
...
function layout(){
	var store=new Ext.data.Store({
		...
		sortInfo:{field:'outWriteTime',direction:'ASC'},
...
//					}else{
//						alert($(this).find('select').eq(0).name);
//						alert(view.getRecord(this).data.outTitle);
//						$(this).find('select').eq(0).attr('value',view.getRecord(this).data.outTitle);
//						alert("yihen");
//					}
//				}
//				var objSele = $(this).find('select').eq(0);
//				objSele.options[objSele.selectedIndex].text = view.getRecord(this).data.outTitle;
...
//					$(this).find('tr').eq(3).children("td").css("display",'none');
	...
	store.baseParams={method:'findAllOutOrMergencyCaseByPatientId',id:PID};
	store.load();
		...
		'<div class="data-item"><div>',
			'<table width="100%" border=0 cellpadding=0 cellspacing=2 name="table{id}">',
				'<tr>',
					...
					'<td><b>&nbsp;&nbsp;门诊时间:{actdate}</b></td>',
					'<td><select {readonly} name="outTitle"><option>{outTitle}</option></select></td>',
...
//					'<img {readonly} src="../../../../PUBLIC/images/calendar.bmp" style="cursor:pointer" onclick="ShowTimePicker(this)"/>',
					'</td>',
				'</tr>',
//				'<tr><td colspan="2" style="padding-left:5px"><a href="" onclick="linkPlan({id})"><font color="red"><B>关联随访</B></font></a></td></tr>',
				'<tr>',
...
//						//liugang 2011-08-06 修改
//							location.href=App.App_Info.BasePath+'/module/Followup/outCaseList.html?id='+PID+'&HISPID='+HISPID;
//						}
//					},'&nbsp;&nbsp;'
				],
...
function ShowTimePicker(_this){
	...
	WdatePicker({el:el,dateFmt:'yyyy-MM-dd'});
}
