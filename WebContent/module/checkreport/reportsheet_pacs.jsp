...
<%
		...
		try {
			...
			report = checkReportDao.getPacsReport(id);
...
<html>
	  ...
	  <div id="sheet">
					...
					<tr>
						...
						<td align="left" valign="middle">
							...
							<table border=0 cellpadding=5 cellspacing=0>
								...
								<tr><td style="font-size:20px;">放射医学影像检查报告单</td></tr>
							</table></div>
						</td>
					</tr>
				</table>
		...
		<div class="block">
			<table border=0 cellpadding=5 cellspacing=0 style="border-bottom:solid 1px #aaaaaa;">
				<tr>
					...
					<td width=300>检查号：<span><%=report.getCheckOrderNum()==null?"":report.getCheckOrderNum() %></span></td>
				</tr>
				<tr>
					<td>检查日期：<span><%=report.getCheckDate() %></span></td>
					<td>报告日期：<span><%=report.getReportDate()==null?"":report.getReportDate() %></span></td>
				</tr>
			</table>
		</div>
		<div class="block">
			<table border=0 cellpadding=5 cellspacing=0 style="border-bottom:solid 1px #aaaaaa;">
				...
				<tr>
					<td>检查部位：<span><%=checkPart %></span></td>
					<td colspan=2>检查项目：<span><%=report.getCheckItem() %></span></td>
				</tr>
				<tr>
					<td colspan=3>报告号：<span><%=report.getReportOrderNum()==null?"":report.getReportOrderNum() %></span></td>
				</tr>
			</table>
		</div>
		<div class="block">
		...
		<div class="block">
			<table border=0 cellpadding=5 cellspacing=0 style="border-bottom:solid 1px #aaaaaa;">
				...
				<tr>
					<td width=300>报告医师：<span><%=report.getReportDoctor()==null?"":report.getReportDoctor() %></span></td>
					<td width=300>审核医师：<span><%=report.getReviewDoctor()==null?"":report.getReviewDoctor() %></span></td>
				</tr>
			</table>
		</div>
		<div class="block">
	...
	function showPlan(paid, repid) {
	   var url = "../../module/plan/selectPlan.html?paId=" + paid + "&repId=" + repid+ "&type=pacs";
