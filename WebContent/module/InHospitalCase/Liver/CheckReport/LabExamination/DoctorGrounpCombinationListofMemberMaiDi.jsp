...
function layout(){
	...
	var ds = new Ext.data.Store({
	    ...
	    ds.baseParams = {
	        method: 'getMyGrounpCheckReportsForMaiDi',
	        search: '',
	        doctorId:top.USER.id
	    };
    ...
    var cm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
    {
        ...
        renderer: function(value, meta, record, rowIndex, colIndex, store){
			...
			escape(record.data.checkItem)+"\",\"" + record.data.guiDangDate + "\",\"" + record.data.sheHeDate + "\")'>查看</a>";
		}
    }, {
        header: '是否矫正修改',
        width: 25,
		...
		'-', {
            ...
            handler: function(){
				...
				if(ss.length==0){
               	...
               	window.open('../../../Liver/CheckReport/inputCombinationList.html?patientId='+ss[0].get('patientId')+'&doctorName='+top.USER.name+"&doctorOrPatient=1", '', 'width=240px;');
            }		
		},
		'-', new SearchButton({
                        handler: function(){
...
function openCheck(id,isPatientOrDoctorWriteZanCun,isFromOutHispital,pId,checkItem,guiDangDate,sheHeDate){
    	...
    	+'&patientId='+pId+'&checkItem='+checkItem+'&guiDangDate='+guiDangDate+'&sheHeDate='+sheHeDate;
		if(isFromOutHispital == 1){
			window.location ='updateLabExaminationMaiDi.html' + params;	
		}	
}   
