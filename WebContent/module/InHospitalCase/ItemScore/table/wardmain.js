...
var inHspDate=App.util.getHtmlParameters('inHspDate');
var title=" "
var indexId = null;
var entity=App.util.getHtmlParameters('entity');
var subScoreItems = "";
var ssmId = "";
var url = App.App_Info.BasePath+'/InHospitalCase/ScoreComment.do?method=getScoreSetMeal';
var ifrPrintUrl = "";
		...
		success:function(_response){
			...
			if(res.success){
				...
				if(btn.menu.items.length==1){
					t=title+'(新增)';
					url=entity+'.html?pid='+pid+'&kid='+kid+'&this=-1&timestamp='+new Date().getTime()+'&inHspDate='+inHspDate;
					...
					url=entity+'.html?pid='+pid+'&kid='+kid+'&this='+lastMenu.id+'&timestamp='+new Date().getTime()+'&inHspDate='+inHspDate;
					url = url + '&subScoreItems=' + subScoreItems + '&title=' + title + '&ssmId=' + ssmId; 
				}
				if(indexId != -1){
					panel.setTitle(t);
...
function indexClick(){
	...
	var url=entity+'.html?pid='+pid+'&kid='+kid+'&this='+this.id+'&timestamp='+new Date().getTime()+'&inHspDate='+inHspDate;
	url = url + '&subScoreItems=' + subScoreItems + '&title=' + title + '&ssmId=' + ssmId; 
	ifrPrintUrl = url;
