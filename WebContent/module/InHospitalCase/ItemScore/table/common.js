...
var array = [];//为了做打印验证，所以将页面组合数组放置在最外边
function initPage(){
	array = [];
	if(subScoreItems){
	...
	for(var i = 0; i < array.length; i++){
		var str = '<iframe id="'+array[i]+'" src="' + array[i] + '.html?subScoreItem=' + array[i] + '&scId=' + thisId + '&caseId=' + kid + '&pid=' + pid + '" style="width:700px;" id="' + array[i] + '"></iframe>';
		$(str).appendTo('body');
	}
