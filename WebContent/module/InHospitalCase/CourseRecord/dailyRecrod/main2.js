...
function layout(){
    var store = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({
            url: baseUrl + '?method=DailyCourseRec_findAllByCaseID&id=' + KID
        }),
    ...
    store.load();
    var tpl = new Ext.XTemplate('<tpl for=".">', '<div class="data-item"><div>', '<table width="100%" border=0 cellpadding=0 cellspacing=2 name="table{id}">', '<tr>', '<td class="time"><input onchange="TimeChange(this)" type="text" name="time" readonly="readonly" value="{time}"/>', '<img {readonly} src="../../../../PUBLIC/images/calendar.bmp" style="cursor:pointer" onclick="ShowTimePicker(this)"/></td>', '<td class="title"><input {readonly} type="text" name="title" value="{title}" onchange="TitleChange(this)"/></td>', '<td class="index">记录号：{orderNo}</td>', '</tr>', '<tr>', '<td class="content" colspan=4><textarea {readonly} name="content" onchange="ContentChange(this)">{content}</textarea></td>', '</tr>', '<tr><td colspan="3">', '<input type="hidden" name="verifyStatus" value="{verifyStatus}"/>', '<input type="hidden" name="submiter" value="{submiter}"/>', '<input type="hidden" name="checker" value="checker"/>', '<input type="hidden" name="submitTime" value="{submitTime}"/>', '<input type="hidden" name="checkTime" value="{checkTime}"/>', '<input type="hidden" name="oldOperation" value="{oldOperation}"/>', '创建{createTime}更新{modifyTime}&nbsp;', '审核状态{verifyStatus_show}&nbsp;&nbsp;&nbsp;&nbsp;', '医生签字', '<input readonly="readonly" style="width:80px" value="{checker_show}">', '<button onclick="CheckSign(this,{id})" class="sign" {check_btn_active}>{check_btn_text}</button>', '<input readonly="readonly" style="width:80px" value="{submiter_show}">', '<button onclick="SubmitSign(this,{id})" class="sign" {submit_btn_active}>{submit_btn_text}</button>', '</td></tr>',    /*'<tr>',
    ...
    var dataview = new Ext.DataView({
        ...
        prepareData: function(data){
            ...
            data.verifyStatus_show = '<img align="absbottom" width="22" height="22" alt="' + imgAlt + '" src="../../../../PUBLIC/images/' + imgSrc + '"/>' +
    ...
    new Ext.Viewport({
        ...
        items: [{
            ...
            tbar: ['-', {
                ...
                menu: [{
                    text: '首次病程记录',
                    handler: addNewRecording
                }, {
                    text: '日常病程记录',
                    handler: addEmptyTitleRecording
                }, {
                    text: '上级查房记录',
                    handler: addNewRecording
                }, {
                ...
                handler: function(){
                    ...
                    if (_record.data.verifyStatus == 1) {
                        ...
                        if (_record.data.verifyStatus == 2) {
                            alert('记录已通过审核，无法删除。');
                            return;
                        }
                    if (!confirm('记录删除后不可恢复，确定要删除？')) 
                        return;
                ...
                text: '化验检查',
                handler: LabExamenation
            }, '-', {
                text: '化验检查(新版)',
                handler: ShowLabExamenation
            }, '-', {
                text: '保存',
                handler: SaveData
            }, '->', {
                            ...
                            render: function(){
                                ...
                                dh.append(this.el.dom, {
                                    ...
                                    children: [{
                                        ...
                                        tag: 'input',
                                        style: 'width:50px;',
										value:Ext.state.Manager.get('CourseRecord_rowOffset_'+KID)||''
                                    }]
                                });
                                dh.append(this.el.dom, {
                                    tag: 'div',
                                    ...
                                    children: [{
                                        ...
                                        tag: 'input',
                                        style: 'width:50px;',
										value:Ext.state.Manager.get('CourseRecord_charOffset_'+KID)||''
                                    }]
                                });
                            }
                        }
...
function SubmitSign(_this, _id){
    ...
    if (_this.innerText == '签字') {
        if (!confirm('确定要签入您的名字<' + top.USER.name + '>？')) 
            return;
        ...
        if (_this.innerText == '撤销') {
            ...
            if (_this.innerText == '重签') {
                if (!confirm('确定要重新签入您的名字<' + top.USER.name + '>？')) 
                    return;
...
function LabExamenation_show(_lab, _index, _record){
    ...
    var returnValue = window.showModalDialog(App.App_Info.BasePath + '/module/InHospitalCase/Liver/CaseDetails/LabExamination.html', _lab ? _lab : {}, winoption);
    if (returnValue) {
