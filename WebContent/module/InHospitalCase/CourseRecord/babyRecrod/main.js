var baseUrl=App.App_Info.BasePath+'/InHospitalCase/BabyRec.do';
		...
		items:[
			{
				...
				tbar:[
					'-',{
						...
						text:'打印',handler:function(){
							window.open('print.html?id='+KID+'&PID='+PID);
						}
					},'->',{
				...
				listeners:{
					render:function(){
						...
						this.body.dom.appendChild(div);
						registrationPageHead(KID,'新生儿记录','#header');
						
						div=document.createElement('div');
						div.id='babyInfo';
						div.innerHTML='<table width="100%" border=0 cellpadding=0 cellspacing=0>'+
						    	'<tr>'+
						    	...
						    	'</tr>'+
						    	'<tr>'+
						    	'	<td style="display:none">出生时间：<input name="birthday" type="text" id="birthDate" style="border-top:0px;border-left:0px;border-right:0px"/></td>'+
						    	'	<td style="display:none">出生时体重：<input type="text" id="birthWeight" style="border-top:0px;border-left:0px;border-right:0px"/></td>'+
						    	'</tr>'+
						    	'</table>'+
						    	'<div class="line-div">----------------------------------------------------------------'+
						    	'----------------------------------------------------</div>';
						div.style.width=700;
						this.body.dom.appendChild(div);
						FormUtil.toDateField({el:$('input[name="birthday"]'),dateFormat:'yyyy-MM-dd HH:mm'});//初始化生日选择框
...
function getBabyInfo(){
	var url= App.App_Info.BasePath+'/InHospitalCase/BabyRec.do'+'?method=getBabyInfo&time=' + new Date().getTime();
	$.getJSON(url,{pid:PID,pcid:KID},function(babyInfo){
	if(babyInfo != 0){
...
function saveBabyInfo(){
	var url= App.App_Info.BasePath+'/InHospitalCase/BabyRec.do'+'?method=saveBabyInfo&time=' + new Date().getTime();
	var name = $("#name").val();
	var gender;
	if($("#gender").val() == "男"){
		gender = 0;
	...
	var babyInfo;
	if(babyId != null){
		babyInfo = '{id:"' + babyId +'",name:"' + name + '",gender:"' + gender + '",birthDate:"' + birthDate + '",birthWeight:"' + birthWeight + '",pid:"' + PID + '",pcid:"' + KID + '"}';		
	}else{
		babyInfo = '{name:"' + name + '",gender:"' + gender + '",birthDate:"' + birthDate + '",birthWeight:"' + birthWeight + '",pid:"' + PID + '",pcid:"' + KID + '"}';		
	}
	hasSave = hasSave + 1;	
	$.getJSON(url,{babyInfo:babyInfo,hasSave:hasSave},function(data){
		getBabyInfo();
		alert(data.type);
	});
}
