var baseUrl=App.App_Info.BasePath+'/InHospitalCase/Anaesthetization.do';
...
function layout(){
	var store=new Ext.data.Store({
		proxy:new Ext.data.HttpProxy({url:baseUrl+'?method=anaesthetizationRec_findAllByCaseID&id='+KID}),
		...
		items:[
			{
				...
				dataUrl:baseUrl+'?method=anaesthetizationRec_treeNodes&id='+KID,
				root:{
					nodeType:'async',
					text:'字典树',
					draggable:false,
				...
				tbar:[
					'-',{
						text:'增加',menu:[
							{
								text:'麻醉医师术前访问',handler:addNewRecording
							},{
								text:'术前诊断',handler:addNewRecording
							},{
								text:'术中小结',handler:addNewRecording
							},{
								text:'术后交接记录',handler:addNewRecording
							},{
								text:'术后访视',handler:addNewRecording
							}
						]
					},'-',{
						text:'删除',handler:function(){
							...
							Ext.Ajax.request({
								url:baseUrl,
								params:{
									method:'anaesthetizationRec_Delete',
									id:_record.data.id
								},
								success:function(_response){
...
function SubmitSign(_this,_id){
	...
	if(_this.innerText=='签字'){
		...
		params.method='anaesthetizationRec_submit';
		params.id=_id;
		...
		params.method='anaesthetizationRec_cancelSubmit';
		params.id=_id;
		callback=function(_response){
			var res=Ext.decode(_response.responseText);
			if(res.success){
		...
		params.method='anaesthetizationRec_resubmit';
		params.id=_id;
		params.submiter=top.USER.id;
		callback=function(_response){
			var res=Ext.decode(_response.responseText);
...
function CheckSign(_this,_id){
	...
	if (_this.innerText == '签字') {
			...
			buttons:[
				{
					text:'提交',handler:function(){
						...
						Ext.Ajax.request({
							url:baseUrl,
							params:{
								method:'anaesthetizationRec_check',
								verifyStatus:v,
								id:_id,
								checker:top.USER.id
							},
		...
		Ext.Ajax.request({
			url:baseUrl,
			params:{
				method:'anaesthetizationRec_cancelCheck',
				id:_id
			},
			success:function(_response){
				var res=Ext.decode(_response.responseText);
...
function SaveData(){
	...
	Ext.Ajax.request({
		url:baseUrl,
		params:{
			method:'anaesthetizationRec_Save',
