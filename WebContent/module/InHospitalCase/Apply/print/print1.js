...
function PREVIEW(_title){
	...
	var  html=document.getElementById('table-div').innerHTML;
	/*
		当页面中有iframe时调用
	*/
	var iframe=$('iframe');
	if(iframe.size()>0){
		$.each(iframe,function(){
			if(this.contentWindow.getPrintTable){
				html+='<p align="center"/>--------------------------------------------------------------------------------'+'<p/>'+'<p/>'+'<p/>'+this.contentWindow.getPrintTable();
			}
		});
	}
	LODOP.ADD_PRINT_HTM(135,80,650,900,html);
