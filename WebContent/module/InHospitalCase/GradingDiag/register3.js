...
//url.getFind = url.baseUrl + 'findGradingDiagByCaseId1';
url.getFind = url.baseUrl + 'findGradingDiagById';
...
function tableInit(){
    ...
    $('<iframe id="print" src="print.jsp" width=0 height=0></iframe><input type="hidden" id="id" value=""/>').appendTo($body);
    $('<input type="hidden" id="cid" value=""/>').appendTo($body);
    $('<input type="hidden" id="fiag" value="N"/>').appendTo($body);
    
    var $table = $('<table class="table" border=0 cellspacing=1 cellpadding=0></table>');
    var $tr = $('<tr><td class="label" width="140">诊断级别：</td><td class="title" width="600">&nbsp;&nbsp;&nbsp;&nbsp;评价时间：&nbsp;&nbsp;&nbsp;<input id="date" type="text" style="width:120px" readonly/><img onclick="WdatePicker({el:$(\'#date\').get(0),dateFmt:\'yyyy-MM-dd HH:mm\'});" class="hand" src="' + App.App_Info.BasePath + '/PUBLIC/images/calendar.bmp" />&nbsp;&nbsp;&nbsp;&nbsp;评价阶段：住院&nbsp;&nbsp;&nbsp;<input name="inOutTimesDate" type="text" class="text" readOnly="readOnly" style="width:100px"/><select id="diagnose" style="200px"><option value=""></option><option value="入院">入院</option><option value="术前">术前</option><option value="术后">术后</option><option value="住院期间">住院期间</option><option value="平时随访">平时随访</option><option value="出院">出院</option></select></td></tr>');
    $tr.appendTo($table);
		...
		$.post(
			...
			function(data){
				if(data.success){
					...
					if(json != null){
					// alert("json"+json.inHspTimes);
					  $('input[name="inOutTimesDate"]').val(json.inHspDate);
					}else{
					  return;
					}
				}
