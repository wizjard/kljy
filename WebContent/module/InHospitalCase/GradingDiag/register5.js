...
var diagnose =App.util.getHtmlParameters('diagnose');
alert(diagnose);
...
function tableInit(){
    ...
    var $tr = $('<tr><td class="label" width="140">诊断级别：</td><td class="title" width="600">&nbsp;&nbsp;&nbsp;&nbsp;评价时间：&nbsp;&nbsp;&nbsp;<input id="date" type="text" style="width:120px" readonly/><img onclick="WdatePicker({el:$(\'#date\').get(0),dateFmt:\'yyyy-MM-dd HH:mm\'});" class="hand" src="' + App.App_Info.BasePath + '/PUBLIC/images/calendar.bmp" />&nbsp;&nbsp;&nbsp;&nbsp;评价阶段：  "\'+diagnose+\'" </td></tr>');
    $tr.appendTo($table);
...
function saveData(){
	...
	//                if(!App.util.getHtmlParameters('id'))
	                parent.refreshNode(res.data,caseId);
	            }
	            else {
	                alert('保存失败。');
	            }
