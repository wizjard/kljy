...
import com.juncsoft.KLJY.InHospitalCase.pitemScore.ItemScore.dao.ScoreCommentPrint1;
...
public class ScoreCommentPrintImpl1 implements ScoreCommentPrint1 {
	...
	public void print(OutputStream os, String root, String kid, String ssmid,
		...
		// 评分阶段
		c = new PdfPCell();
		p = new Paragraph(" 评价阶段：门诊(" + data.get("评分阶段")+")", myFont);
		p.setAlignment(Element.ALIGN_CENTER);
		c.addElement(p);
		c.setUseAscender(true);
		c.setVerticalAlignment(Element.ALIGN_MIDDLE);
...
//		}
	...
	private PdfPTable getHeader() throws DocumentException,
		...
		cell.setUseAscender(true);
		cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
		p = new Paragraph("第 " + data.get("门诊次数") + " 次门诊", myFont);
		cell.addElement(p);
		table.addCell(cell);
		// 补白
		cell = new PdfPCell();
	...
	private void preparData() throws Exception {
		...
		try {
			...
			// 病人门诊信息
			if(data.get("kid").length() >=10){
				String hql = "from MemberInfo where memberno = '"+data.get("kid")+"'";
						...
						if(outList!= null && outList.size() > 0){
							...
							if(outOrMergencyCase != null){
								if(outOrMergencyCase.getOutCount() != null){
									data.put("门诊次数", outOrMergencyCase.getOutCount() + "");
								}else{
									data.put("门诊次数", 1+"");
								}
								data.put("评分阶段", dateFormat.format(outOrMergencyCase.getActdate())+"");
							}
						}
						data.put("姓名", patient.getPatientName());
						data.put("病案号", patient.getPatientNo());
					}
				}
			}else{
				OutOrMergencyCase mc = (OutOrMergencyCase) session.get(
						OutOrMergencyCase.class, Long.parseLong(data.get("kid")));
				if (mc != null) {
					if(mc.getOutCount() != null){
						data.put("门诊次数", mc.getOutCount() + "");
					}else{
						data.put("门诊次数", 1 + "");
					}
					
					data.put("评分阶段", dateFormat.format(mc.getOutTime()));
					Patient p = (Patient) session.get(Patient.class, Long.parseLong(mc.getPatientId()));
					if (p != null) {
			...
			if (scoreComment != null) {
				data.put("评分时间", dateFormat.format(scoreComment
								.getScoreTime()));
	...
	private void preparPageData(String pageflag, Session session)
		...
		try {
			...
			if (pageflag.equals("Child-Pugh")) {
				entity = impl.childPugh_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("MELD")) {
				entity = impl.meld_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("BCLC")) {
				entity = impl.bclc_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("BMI")) {
				entity = impl.bmi_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("CLIP")) {
				entity = impl.clip_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("GCS")) {
				entity = impl.gcs_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("PLC")) {
				entity = impl.plc_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("TNM")) {
				entity = impl.tnm_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("CTOS")) {
				entity = impl.lctos_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("HE")) {
				entity = impl.he_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("OrganFunc")) {
				entity = impl.organFunc_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("HC")) {
				entity = impl.hc_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("HRS")) {
				entity = impl.hrs_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("HPS")) {
				entity = impl.hps_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("Hepatomyocardosis")) {
				entity = impl.hepatomyocardosis_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("APACHEII")) {
				entity = impl.apacheii_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("PHI")) {
				entity = impl.phi_findByScidmenzhen(session, scid);
			} else if (pageflag.equals("TS")) {
				entity = impl.ts_findByScidmenzhen(session, scid);
			}
