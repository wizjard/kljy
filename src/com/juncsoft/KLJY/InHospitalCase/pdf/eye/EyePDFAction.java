...
import com.juncsoft.KLJY.InHospitalCase.pdf.PhysicalExaminationPDF;
...
public class EyePDFAction extends DispatchAction{
	...
	private PhysicalExaminationPDF phy = new PhysicalExaminationPDF();
	private LabDiagnosticExaminationPDF lab = new LabDiagnosticExaminationPDF();
	private DiagnosisPDF diag = new DiagnosisPDF();
	private EyePDF eye = new EyePDF();
	public ActionForward eyePDF_Print(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {

		Long id = Long.parseLong(request.getParameter("id"));
		JSONObject json = null;
		// dao.InHspDied24_print(id);
...
//			cellC.setLeft(20);
			...
			//专科检查眼科
			PdfPTable tableFour = new PdfPTable(5);
			tableFour.setSplitLate(false);
			tableFour.setSplitRows(true);
			// 设置表格宽度100%
			tableFour.setWidthPercentage(100);
			tableFour.setWidths(new int[] {12,22,22,22,22});
		
			eye.getContent(tableFour, fontGeneral, myFontItem, id, session, map, df, filePath);
			cell.addElement(tableFour);
						
			for (PdfPRow row : tableFour.getRows()) {
				for (PdfPCell c : row.getCells()) {
					if (c != null)
						c.setBorder(Rectangle.NO_BORDER);
				}
			...
			PdfPTable tableFive = new PdfPTable(5);
			tableFive.setSplitLate(false);
			tableFive.setSplitRows(true);
			// 设置表格宽度100%
			tableFive.setWidthPercentage(100);
			tableFive.setWidths(new int[] {13,19,25,19,15});
			//辅助检查
			lab.getContent(tableFive, fontGeneral, myFontItem, id, session, map, df);
			//确定诊断/初步诊断/订正诊断
			diag.getContent(tableFive, fontGeneral, myFontItem, id, session, map, df);
			cell.addElement(tableFive);
			
			for (PdfPRow row : tableFive.getRows()) {
				for (PdfPCell c : row.getCells()) {
					if (c != null)
						c.setBorder(Rectangle.NO_BORDER);
				}
