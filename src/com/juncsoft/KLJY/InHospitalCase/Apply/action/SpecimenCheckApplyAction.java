...
import com.juncsoft.KLJY.InHospitalCase.Apply.impl.SpecimenCheckApplyDaoImpl;
import com.juncsoft.KLJY.util.Response;

public class SpecimenCheckApplyAction extends DispatchAction {
	private SpecimenCheckApplyDao dao = new SpecimenCheckApplyDaoImpl();
	...
	public ActionForward saveOrUpdate(ActionMapping mapping, ActionForm form,
		...
		Long id = Long.parseLong(request.getParameter("id"));
		PrintWriter out = response.getWriter();
		SpecimenCheckApply entity = (SpecimenCheckApply) JSONObject.toBean(JSONObject
				.fromObject(data), SpecimenCheckApply.class);
		String backId = null;
		try {
	...
	public ActionForward deleteData(ActionMapping mapping, ActionForm form,
		...
		String data = request.getParameter("data");
		try {
			SpecimenCheckApply entity = (SpecimenCheckApply) JSONObject.toBean(
					JSONObject.fromObject(data), SpecimenCheckApply.class);
	...
	// 打印
	public ActionForward SpecimenCheckApplyPrint(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		Long id = Long.parseLong(request.getParameter("id"));
		PrintWriter out = response.getWriter();
		...
		try {
			if (id == 0) {
				...
				res.setData(JSONObject.fromObject(dao.SpecimenCheckApply_print(id)).toString());
				res.setSuccess(true);
			}
		} catch (Exception e) {
			res.setSuccess(false);
