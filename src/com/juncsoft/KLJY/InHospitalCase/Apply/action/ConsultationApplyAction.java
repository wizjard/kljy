...
import com.juncsoft.KLJY.InHospitalCase.Apply.impl.ConsultationApplyDaoImpl;
import com.juncsoft.KLJY.util.Response;

public class ConsultationApplyAction extends DispatchAction {
	private ConsultationApplyDao dao = new ConsultationApplyDaoImpl();
	...
	public ActionForward saveOrUpdate(ActionMapping mapping, ActionForm form,
		...
		String backId = null;
		try {
			ConsultationApply entity = (ConsultationApply) JSONObject.toBean(JSONObject
					.fromObject(data), ConsultationApply.class);
	...
	public ActionForward deleteData(ActionMapping mapping, ActionForm form,
		...
		String data = request.getParameter("data");
		try {
			ConsultationApply entity = (ConsultationApply) JSONObject.toBean(
					JSONObject.fromObject(data), ConsultationApply.class);
	...
	// 打印
	public ActionForward ConsultationApplyPrint(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		Long id = Long.parseLong(request.getParameter("id"));
		PrintWriter out = response.getWriter();
		...
		try {
			if (id == 0) {
				...
				res.setData(JSONObject.fromObject(dao.ConsultationApply_print(id)).toString());
				res.setSuccess(true);
			}
		} catch (Exception e) {
			res.setSuccess(false);
