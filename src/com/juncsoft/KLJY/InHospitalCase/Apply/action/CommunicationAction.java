...
import com.juncsoft.KLJY.InHospitalCase.Apply.impl.CommunicationDaoImpl;
import com.juncsoft.KLJY.util.Response;

public class CommunicationAction extends DispatchAction {
	private CommunicationDao dao = new CommunicationDaoImpl();
	...
	public ActionForward get_Patient_Info(ActionMapping mapping,
		...
		System.out.println("The method of get_Patient_Info was running....");
	...
	public ActionForward saveOrUpdate(ActionMapping mapping, ActionForm form,
		...
		try {
			if (data != null) {
				Communication entity = (Communication) JSONObject.toBean(
						JSONObject.fromObject(data), Communication.class);
	...
	public ActionForward deleteData(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		System.out.println("The method of deleteDate was running....");
		String data = request.getParameter("data");
		System.out.println(data);
		try {
			Communication entity = (Communication) JSONObject.toBean(JSONObject
					.fromObject(data), Communication.class);
	...
	// 打印
	public ActionForward CommunicationPrint(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		Long id = Long.parseLong(request.getParameter("id"));
		PrintWriter out = response.getWriter();
		...
		try {
			if (id == 0) {
				...
				res.setData(JSONObject.fromObject(dao.Communication_print(id)).toString());
				res.setSuccess(true);
			}
		} catch (Exception e) {
			res.setSuccess(false);
