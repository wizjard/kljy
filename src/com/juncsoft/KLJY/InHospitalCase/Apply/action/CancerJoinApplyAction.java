...
import com.juncsoft.KLJY.InHospitalCase.Apply.impl.CancerJoinApplyDaoImpl;
import com.juncsoft.KLJY.util.Response;

public class CancerJoinApplyAction extends DispatchAction {
	private CancerJoinApplyDao dao = new CancerJoinApplyDaoImpl();
	...
	public ActionForward saveOrUpdate(ActionMapping mapping, ActionForm form,
		...
		Long id = Long.parseLong(request.getParameter("id"));
		PrintWriter out = response.getWriter();
		CancerJoinApply entity = (CancerJoinApply) JSONObject.toBean(JSONObject
				.fromObject(data), CancerJoinApply.class);
		String backId = null;
		try {
	...
	public ActionForward deleteData(ActionMapping mapping, ActionForm form,
		...
		String data = request.getParameter("data");
		try {
			CancerJoinApply entity = (CancerJoinApply) JSONObject.toBean(
					JSONObject.fromObject(data), CancerJoinApply.class);
	...
	// 打印
	public ActionForward CancerJoinApplyPrint(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		Long id = Long.parseLong(request.getParameter("id"));
		PrintWriter out = response.getWriter();
		...
		try {
			if (id == 0) {
				...
				res.setData(JSONObject.fromObject(dao.CancerJoinApply_print(id)).toString());
				res.setSuccess(true);
			}
		} catch (Exception e) {
			res.setSuccess(false);
