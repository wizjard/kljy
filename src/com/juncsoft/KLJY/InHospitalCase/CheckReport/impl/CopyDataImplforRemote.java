...
public class CopyDataImplforRemote implements CopyDataDao {
	...
	public boolean copyReportInfo() {
		...
		//创建临时表并插入数据(如果存在则先删除)
		String dropTempTable = "drop table ##__ReportForm";
		String createTempTable = "create table ##__ReportForm(id int IDENTITY (1, 1) NOT NULL ,ReceiveDate datetime NOT NULL ," +
		...
		String insertTempTable = "insert into ##__ReportForm select ReceiveDate,SectionNo,TestTypeNo,SampleNo,StatusNo," +
	...
	private int getCount() throws SQLException {
		...
		String sql = "select count(*) from ##__ReportForm ";
		try {
...
//				statement_toRemote.setInt(2, i * P_N);
				sqlForSelect = "select top " + P_N + " * from ##__ReportForm where id not " +
					"in (select top " + i * P_N + " id from ##__ReportForm order by id) order by id";
				statement_toRemote = con_toRemote.prepareStatement(sqlForSelect);
